...
int			mlx_loop(t_xvar *xvar)
{
	...
	while (win_count(xvar) && !xvar->end_loop)
	{
		while (!xvar->end_loop && (!xvar->loop_hook || XPending(xvar->display)))
		{
			...
			if (win && ev.type == ClientMessage && ev.xclient.message_type == xvar->wm_protocols && (long int)ev.xclient.data.l[0] == (long int)xvar->wm_delete_window && win->hooks[DestroyNotify].hook)
